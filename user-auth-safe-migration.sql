...
-- 15. 创建数据迁移视图（用于兼容现有应用）
CREATE OR REPLACE VIEW public.travel_plan_compat AS
SELECT 
    tp.*,
    COALESCE(tp.app_user_id, au.id) as effective_user_id
FROM public.travel_plans tp
LEFT JOIN public.app_users au ON tp.app_user_id = au.id;
...
-- 显示迁移完成信息
SELECT '用户认证数据库安全迁移完成！' as migration_status;
SELECT '注意：现有user_id列保持不变，新增app_user_id列用于新认证系统' as migration_note;